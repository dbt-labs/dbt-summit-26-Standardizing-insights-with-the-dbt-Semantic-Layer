...
with
    order_item as (select * from {{ ref("int_order_items_joined") }}),

    part_supplier as (select * from {{ ref("int_part_suppliers_joined") }}),
    ...
    final as (
        ...
        inner join
            part_supplier
            on order_item.part_id = part_supplier.part_id
            and order_item.supplier_id = part_supplier.supplier_id
    )
